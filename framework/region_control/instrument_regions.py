...
after --. Application-specific timing windows and step callbacks remain explicit.
"""
import argparse
...
def clang_ast(source, original, args, clang):
    command = [clang, "-fopenmp", "-fsyntax-only", "-Xclang", "-ast-dump=json",
               "-iquote", str(original.parent), *args, str(source)]
...
def generate(sources, regions, barriers, prefix, adapter_header):
    """Emit application-prefixed hooks and metadata around unchanged pragmas."""
    upper, lower = prefix.upper(), prefix.lower()
    files = {}
    header = ['/* Generated by instrument_regions.py; edit the original sources. */',
              f'#ifndef {upper}_GENERATED_REGIONS_H', f'#define {upper}_GENERATED_REGIONS_H',
              f'#include "{adapter_header}"', 'enum {']
    header += [f'  R_AUTO_{r.id} = {r.id},' for r in regions]
    header += ['  R_AUTO_COUNT', '};',
               f'#define {upper}_AUTO_FOR_START(id) \\',
               f'  if ({lower}_time_active) {{ {upper}_TIME_MASTER {lower}_time_nowait_start(id); }}',
               f'#define {upper}_AUTO_SYNC() \\',
               f'  if ({lower}_time_active) {{ {upper}_TIME_MASTER {lower}_time_sync(); }}',
               '#endif', '']
    files[f'{lower}_generated_regions.h'] = '\n'.join(header)
    table = [f'#include "{lower}_generated_regions.h"',
             f'const {lower}_region_info {lower}_regions[] = {{']
    for r in regions:
    ...
    table += ['};', f'const int {lower}_region_count = {len(regions)};', '']
    files[f'{lower}_generated_regions.c'] = '\n'.join(table)
    for path, source in sources.items():
        ...
        for r in regions:
            ...
            if r.nowait:
                ...
                begin = f'{upper}_AUTO_FOR_START(R_AUTO_{r.id})'
                end = ''
                if not r.direct:
                    begin, end = '{\n' + begin, '}'
            else:
                macro = upper + {'parallel': '_PARALLEL', 'combined': '_PARALLEL_FOR', 'for': '_FOR'}[r.kind]
                begin, end = f'{macro}_BEGIN(R_AUTO_{r.id})', f'{macro}_END()'
        ...
        for start, end in barriers.get(path, []):
            ...
            edits[end].append((0, -start, f'{upper}_AUTO_SYNC()\n}}'))
        result = source
        ...
        files[path.name] = ('/* Generated by instrument_regions.py. */\n'
                            f'#include "{lower}_generated_regions.h"\n'
...
def main():
    ...
    parser.add_argument('--adapter-header', default='region_control_adapter.h')
    parser.add_argument('--prefix', default='NPB', help='metadata prefix (lowercase) and macro prefix (uppercase)')
    parser.add_argument('--include-dir', type=Path, action='append', default=[],
                        help='adapter/build include directory; may be repeated')
    parser.add_argument('--strip-existing', action='store_true',
                        help='remove standalone prefixed BEGIN/END lines, retaining application window markers')
    import sys
    argv = sys.argv[1:]
    split = argv.index('--') if '--' in argv else len(argv)
    args = parser.parse_args(argv[:split])
    flags = argv[split + 1:] if split < len(argv) else []
    flags = [value for path in args.include_dir for value in ('-I', str(path.resolve()))] + flags
    upper, lower = args.prefix.upper(), args.prefix.lower()
    pairs = re.compile(r'^[ \t]*' + re.escape(upper) +
                       r'_(?:PARALLEL_FOR|PARALLEL|FOR)_(?:BEGIN\([^\n]*\)|END\(\))[ \t]*$',
                       re.MULTILINE)
    ...
    try:
        for path in args.sources:
            ...
            if path.suffix not in ('.c', '.cc', '.cpp', '.cxx'):
                raise ValueError(f'{path}: expected a .c, .cc, .cpp or .cxx source file')
            if path.name == f'{lower}_generated_regions.c':
                raise ValueError(f'{path.name} is reserved for the generated table')
            ...
            if re.search(re.escape(lower) + r'_time_(?:start|stop)\s*\(', text):
        ...
        with tempfile.TemporaryDirectory(prefix='region-instrument-') as temporary:
            ...
            for index, region in enumerate(regions):
                region.id = index
            if len(regions) > 256:
                raise ValueError(f'{len(regions)} regions exceed the timer limit of 256')
            assign_parents(regions, calls)
            files = generate(sources, regions, barriers, args.prefix, args.adapter_header)
            for name, text in files.items():
                (temporary / name).write_text(text)
            for path in sources:
                command = [args.clang, '-fopenmp', '-fsyntax-only', '-I', str(temporary),
                           '-iquote', str(path.parent), *flags,
            ...
            manifest = {'generator': GENERATOR, 'sources': list(map(str, sources)), 'compiler_args': flags,
                        'prefix': args.prefix, 'adapter_header': args.adapter_header,
            ...
            print(f'Compile generated sources with {output}/{lower}_generated_regions.c and your adapter;')
            print('retain the original include/define flags and adapter include directories')
