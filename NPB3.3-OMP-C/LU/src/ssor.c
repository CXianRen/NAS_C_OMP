...
void ssor(int niter, logical timed_iterations)
{
  ...
  if (timed_iterations) iteration_start(&benchmark_control);

  //---------------------------------------------------------------------
  // the timestep loop
  //---------------------------------------------------------------------
  for (istep = 1; istep <= niter; istep++) {
    if (timed_iterations) step_start(&benchmark_control, istep);
    ...
    //---------------------------------------------------------------------
    if (timed_iterations) step_end(&benchmark_control, istep);
  ...
  if (timed_iterations) iteration_end(&benchmark_control);
  maxtime = iteration_time(&benchmark_control);
}
