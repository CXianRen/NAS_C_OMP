...
int32 DC(ADC_VIEW_PARS *adcpp) {
  ...
  step_start(&benchmark_control, 1);
#pragma omp parallel shared(pvstp) private(itsk)
  {
   ADC_VIEW_CNTL *adccntlp;
   /* A tuner may change the team at parallel entry. Partition using this
      actual team; thread-local copies avoid shared parameter writes. */
   ADC_VIEW_PARS team_pars = *adcpp;
#ifdef _OPENMP
   itsk=omp_get_thread_num();
   team_pars.nTasks=omp_get_num_threads();
#endif
   if (itsk == 0) actual_tasks = team_pars.nTasks;
   adccntlp = NewAdcViewCntl(&team_pars, itsk);
...
#pragma omp critical
   ...
   adcpp->nTasks = actual_tasks;
   step_end(&benchmark_control, 1);
  iteration_end(&benchmark_control);

   t_total=iteration_time(&benchmark_control);
